  ...
  public func runTests() : async () {
    await suite(
      ...
      func() : async () {
        ...
        await test(
          "Should create new attachment store",
          func() : async () {
        ...
        await test(
          "Should write and get metadata for small attachment",
          func() : async () {
            let store = DiodeAttachments.new(1000);
            let hash = make_hash(1);
            let data = Blob.fromArray([1, 2, 3, 4, 5]);
        ...
        await test(
          "Should write and read small attachment",
          func() : async () {
            let store = DiodeAttachments.new(1000);
            let hash = make_hash(1);
            let data = Blob.fromArray([1, 2, 3, 4, 5]);
        ...
        await test(
          "Should handle large attachments using multi-step process",
          func() : async () {
            let store = DiodeAttachments.new(1000);
        ...
        await test(
          "Should handle finalized behavior correctly",
          func() : async () {
            let store = DiodeAttachments.new(1000);
        ...
        await test(
          "Should handle deletion correctly",
          func() : async () {
            let store = DiodeAttachments.new(1000);
        ...
        await test(
          "Should handle ring-buffer edge cases at buffer boundaries #1",
          func() : async () {
        ...
        await test(
          "Should handle ring-buffer edge cases at buffer boundaries #2",
          func() : async () {
            ...
            let data3 = Blob.fromArray(Array.tabulate<Nat8>(952, func i = Nat8.fromIntWrap(i))); // 953 + 48 = 1001
        ...
        await test(
          "Should handle ring-buffer edge cases at buffer boundaries #3",
          func() : async () {
            // Test case 3: size + metadata_size == max_offset + 1 (should trigger wrap)
            let store3 = DiodeAttachments.new(1000);
            let hash3 = make_hash(12);
            let data3 = Blob.fromArray(Array.tabulate<Nat8>(951, func i = Nat8.fromIntWrap(i))); // 953 + 48 = 1001
        ...
        await test(
          "Should handle multiple ring-buffer loops with different sizes",
          func() : async () {
            ...
            assert not attachmentExists(DiodeAttachments.get_attachment(store, hash3));
        ...
        await test(
          "Should handle ring-buffer with exact boundary calculations",
          func() : async () {
            ...
            let data1 = Blob.fromArray(Array.tabulate<Nat8>(951, func i = Nat8.fromIntWrap(i)));

            // This should fit exactly: 951 + 48 = 999
            assert isOk(DiodeAttachments.write_attachment(store1, hash1, data1));

            // Test 2: Try to write one more byte (should trigger wrap)
            let hash2 = make_hash(31);
            let data2 = Blob.fromArray(Array.tabulate<Nat8>(952, func i = Nat8.fromIntWrap(i)));

            // This should trigger ring-buffer wrap: 952 + 48 = 1000
        ...
        await test(
          "Should handle error cases correctly",
          func() : async () {
            let store = DiodeAttachments.new(1000);
        ...
        await test(
          "Should handle chunk reading correctly",
          func() : async () {
            let store = DiodeAttachments.new(1000);
